...
// Available reports whether the WebView2 runtime is installed and usable on
// this system. It uses the official GetAvailableCoreWebView2BrowserVersionString
// API (via the webviewloader helper), which returns an empty string when the
// runtime is not installed. This is a cheap, non-blocking, panic-free probe —
// crucial because the go-webview2 library crashes if you try to create a
// webview without a runtime.
func Available() bool {
	ver, err := webviewloader.GetInstalledVersion()
	if err != nil {
		// The loader itself failed (e.g., WebView2Loader.dll missing). Treat
		// as unavailable so callers fall back to the browser.
		return false
	}
	// Empty version string means the runtime is not installed.
	return ver != ""
}

// InstalledVersion returns the WebView2 runtime version string (e.g.
// "131.0.2903.86") or empty if not installed. Useful for diagnostics.
func InstalledVersion() string {
	ver, _ := webviewloader.GetInstalledVersion()
	return ver
}
