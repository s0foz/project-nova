...
const (
	...
	// ---- Runner / inference backend ----
	// EnvRunner selects the inference backend: "auto" (default), "llamacpp",
	// or "stub". "auto" uses llama.cpp if llama-server is found, else stub.
	EnvRunner = "NOVA_RUNNER"
	// EnvLlamaServerPath is an explicit path to the llama-server binary.
	// If unset, Nova searches PATH, the executable's directory, and ./bin.
	EnvLlamaServerPath = "NOVA_LLAMA_SERVER_PATH"
	// EnvLlamaServerArgs is extra CLI args appended to the llama-server
	// invocation (e.g. "--cont-batching --no-context-shift").
	EnvLlamaServerArgs = "NOVA_LLAMA_SERVER_ARGS"

	// ---- Model sources ----
	// EnvHfToken is sent as a Bearer token when pulling gated HuggingFace
	// models. The HF_TOKEN env var is also honoured.
	EnvHfToken = "NOVA_HF_TOKEN"
)

// RunnerName returns the configured runner backend ("auto" by default).
func RunnerName() string {
	if v := os.Getenv(EnvRunner); v != "" {
		return v
	}
	return "auto"
}

// LlamaServerPath returns an explicit llama-server path override, or "".
func LlamaServerPath() string { return os.Getenv(EnvLlamaServerPath) }

// LlamaServerArgs returns extra args for llama-server (whitespace-split).
func LlamaServerArgs() string { return os.Getenv(EnvLlamaServerArgs) }

// HfToken returns the HuggingFace access token, preferring NOVA_HF_TOKEN then
// HF_TOKEN.
func HfToken() string {
	if t := os.Getenv(EnvHfToken); t != "" {
		return t
	}
