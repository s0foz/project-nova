...
external client (curl, OpenAI SDK, etc.) talk to. It also serves the embedded
chat web UI at `http://<host>/`.
...
Start the Nova API server (identical to `nova serve`) and then open the
embedded chat UI in a **native desktop window**. This is the fastest way from
"Nova installed" to "I am chatting with a model in a window."

On Windows the UI renders inside a **WebView2 window** — a real application
window with no browser chrome (no address bar, no tabs, no "open in new tab").
It looks and behaves like a native app. If the WebView2 runtime is not
installed (rare on Windows 11, possible on older Windows 10 builds), Nova
falls back to opening `http://<host>/` in the system default browser.

The chat UI is a dark-mode single-page app with: model picker, streaming
responses, system-prompt editor, temperature control, markdown rendering with
code copy buttons, and localStorage persistence of your conversation. It talks
to the local Nova server over HTTP.
...
| Flag | Type | Default | Description |
| --- | --- | --- | --- |
| `--host`, `-H` | string | `$NOVA_HOST` or `127.0.0.1:11434` | Listen address |
| `--browser` | bool | `false` | Open the UI in the system browser instead of a desktop window |
| `--no-browser` | bool | `false` | Start the server without opening any window (headless / CI) |

#### Examples

```bash
# Start the server and open the chat UI in a desktop window
nova chat

# Force the old browser-tab behaviour
nova chat --browser

# Headless — just start the server (equivalent to `nova serve`)
nova chat --no-browser

# Custom port
nova chat -H 127.0.0.1:8080
```

#### Notes

- If no models are installed yet, the UI shows an empty state with the command
  to pull one (`nova pull hf:owner/repo/model.gguf`).
- To get **real** AI output (not the echo stub), install `llama-server` from
  [llama.cpp releases](https://github.com/ggml-org/llama.cpp/releases) and set
  `NOVA_LLAMA_SERVER_PATH`, or place `llama-server.exe` next to `nova.exe`.
- The UI is also available any time the server is running by visiting
  `http://127.0.0.1:11434/` in your browser, or via the tray app's
  **Open Chat** menu item (which also opens a native desktop window).
- Closing the desktop window shuts down the server and exits `nova chat`. To
  keep the server running in the background, use `nova serve` or the tray app
  instead, and open the window from the tray.
...
#### Pulling real models (HuggingFace / direct URL / local file)

Nova has no upstream registry yet, but it can download real GGUF models from
several sources. The "name" argument is interpreted as a **source specifier**
when it starts with one of these prefixes:

| Source form | Where it downloads from |
| --- | --- |
...
| `/abs/path/model.gguf` or `C:\...` | local file (Unix / Windows) |

When a source specifier is used, Nova derives a local model name from the
filename (e.g. `tinyllama-1.1b-chat.Q4_K_M.gguf` → `tinyllama-1.1b-chat`).
...
nova pull https://example.com/models/qwen2-7b.gguf

# Register a local file you already downloaded
nova pull file:///C:/Users/aman/Downloads/model.gguf

# Gated HuggingFace models need a token:
$env:NOVA_HF_TOKEN = "hf_xxx"
nova pull hf:meta-llama/Llama-3-8B-Instruct-GGUF/llama-3-8b-instruct.Q4_K_M.gguf
```
...
```
pulling manifest...
pulling tinyllama   100% |████████████████████| 4.7 GB   12.3 MB/s   6m18s
