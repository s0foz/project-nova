...
embedded chat web UI in your default browser. This is the fastest way from
...
code copy buttons, and localStorage persistence of your conversation. It is
served at `http://<host>/` and talks to the local Nova server over HTTP.
...
| `--no-browser` | bool | `false` | Start the server without opening a browser (headless / CI) |

#### Examples

```bash
# Start the server and open the chat UI
nova chat
