...
- 💻 **CLI** — `nova serve`, `nova run`, `nova chat`, `nova pull`, `nova list`, `nova rm`,
...
- 🌐 **Real model pulling** — `nova pull hf:owner/repo/file.gguf` downloads
  from HuggingFace; also supports direct `https://`/`http://` URLs and local
  `file://`/absolute paths. Streaming progress + sha256 verification.
- 🧠 **llama.cpp runner** — Nova spawns `llama-server` (from llama.cpp) as a
  subprocess for real local inference. Auto-detected when installed; falls back
  to the echo stub when not. Configurable via `NOVA_RUNNER`,
  `NOVA_LLAMA_SERVER_PATH`, `NOVA_LLAMA_SERVER_ARGS`.
- 💬 **Built-in chat UI** — a polished dark-mode web app served at `http://localhost:11434/`.
  Model picker, streaming chat, system prompts, markdown rendering, code copy
  buttons. Opens in a **native desktop window** (WebView2) via `nova chat` or
  the tray's "Open Chat" item — no browser tab, no address bar, just an app
  window. Falls back to the browser if WebView2 isn't installed.
- ⚙️ **Configurable** — env vars for host, models dir, CORS origins, debug,
  keep-alive, max runners, VRAM limits, flash-attention, context size, runner.
- 🪟 **Tray app** — `nova-tray.exe` runs in the system tray with no console
  window; right-click for **Open Chat**, **Open Logs Folder**, or **Quit**.
...
Nova works out of the box with the stub runner (echoes input), and switches to
real inference automatically once `llama-server` is installed. Three ways to
get a model:
...
# 1) Pull a real GGUF from HuggingFace (downloads + verifies sha256):
nova pull hf:TheBloke/TinyLlama-1.1B-Chat-v1.0-GGUF/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf

# 2) Pull from a direct URL:
nova pull https://example.com/models/tiny.gguf

# 3) Register a local file:
nova pull file:///C:/Users/aman/Downloads/model.gguf

nova list                   # list installed models
nova run tinyllama "Hello!"# stream a completion
nova chat                   # open the web UI in your browser
```

Sample output from `nova run tinyllama "Hello!"`:

```
Pulling manifest...
Pulling tinyllama   100% |████████████████████| 637 MB
Verifying sha256...
Writing manifest...
Success
[Nova stub] Echo: Hello!      # stub runner (no llama-server installed)
```

To enable **real inference**, install llama.cpp and point Nova at it:

```powershell
# Download llama-bundle-win-x64.zip from https://github.com/ggerganov/llama.cpp/releases
# Extract llama-server.exe next to nova.exe, or set:
$env:NOVA_LLAMA_SERVER_PATH = "C:\Tools\llama.cpp\llama-server.exe"
nova run tinyllama "Hello!"  # now generates real text
```
...
nova chat                      # start the server AND open the chat UI in your browser
nova --tray                    # start the desktop tray app (or run nova-tray.exe)
nova pull hf:owner/repo/m.gguf # pull a real GGUF from HuggingFace
nova pull https://x.com/m.gguf # pull from a direct URL
nova pull llama3               # bare name (needs an upstream registry — not yet)
...
| `NOVA_RUNNER` | `auto` | Inference backend: `auto` (llama.cpp if found, else stub), `llamacpp`, or `stub` |
| `NOVA_LLAMA_SERVER_PATH` | (search) | Explicit path to the `llama-server` binary |
| `NOVA_LLAMA_SERVER_ARGS` | (unset) | Extra CLI args appended to llama-server (e.g. `--cont-batching`) |
| `NOVA_HF_TOKEN` / `HF_TOKEN` | (unset) | Bearer token for pulling gated HuggingFace models |
...
double-clicking it won't pop up a console window. It starts the API server
in-process and runs in the system tray with a right-click menu:

- **Project:Nova** — header.
- **Listening on \<host\>** — shows the bound address (disabled).
- **Open Chat** — opens the chat UI in a **native desktop window** (WebView2). Spawns a separate `nova --window` process so the tray stays responsive; closing the window doesn't stop the server.
- **Open Logs Folder** — opens the logs directory in Explorer.
- **Quit** — cleanly unloads all models and exits.
